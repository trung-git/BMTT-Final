...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  const text = `Hi ${newUser.name.split(' ')[0]},
  Thank you for signing up to our application 🤩
  Enjoy 🖤`;
  await sendMail(req, 'Welcome to COCOTON-E', text, newUser.email);
  // Success and send token
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const text = `Hi there,
    Click this link to reset your password (valid in 10 minutes)
    ${resetURL}`;
    await sendMail(req, 'Reset your password', text);
